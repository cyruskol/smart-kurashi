        ...
        <section style={{ background: '#FAFAF9', color: '#292524', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="max-w-container mx-auto px-md" style={{ padding: '64px 0', position: 'relative', zIndex: 1 }}>
            <div style={{ maxWidth: '720px' }}>
              <span style={{ display: 'inline-block', padding: '4px 12px', background: '#C2703E', color: '#fff', fontSize: '11px', fontWeight: 600, borderRadius: '8px', marginBottom: '16px', letterSpacing: '0.05em' }}>
                ⭐ 注目記事
              </span>
                      ...
                      {post.tags.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '12px' }}>
                          {post.tags.slice(0, 3).map((tag) => (
                            <span key={tag} style={{ padding: '2px 8px', background: '#F1F5F9', color: '#78716C', fontSize: '10px', fontWeight: 500, borderRadius: '8px' }}>
                              {tag}
                            </span>
                          ))}
                        </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #E7E5E4', borderRadius: '8px', padding: '20px', marginBottom: '24px' }}>
              ...
              <form action="/search" method="GET">
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input type="search" name="q" placeholder="キーワード..." style={{ flex: 1, padding: '8px 12px', border: '1px solid #E7E5E4', borderRadius: '8px', fontSize: '13px', background: '#F8FAFC' }} />
            ...
            <div style={{ background: '#fff', border: '1px solid #E7E5E4', borderRadius: '8px', padding: '20px', marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {popularTags.map(([tag, count]) => (
                  <Link key={tag} href={`/search?q=${encodeURIComponent(tag)}`} style={{ padding: '4px 12px', background: '#F1F5F9', color: '#78716C', fontSize: '12px', fontWeight: 500, borderRadius: '8px', textDecoration: 'none' }} className="hover:bg-orange-100 hover:text-orange-600 transition-colors">
                ...
                {allPosts.slice(0, 6).map((post, i) => (
                  <li key={post.slug} style={{ padding: '10px 0', borderBottom: i < 5 ? '1px solid #F1F5F9' : 'none' }}>
                    <Link href={`/posts/${post.slug}`} style={{ display: 'flex', gap: '10px', textDecoration: 'none', alignItems: 'flex-start' }}>
                      <span style={{ width: '24px', height: '24px', background: i < 3 ? '#E8643A' : '#F1F5F9', color: i < 3 ? '#fff' : '#78716C', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 600, flexShrink: 0 }}>
                        {i + 1}
                      </span>
